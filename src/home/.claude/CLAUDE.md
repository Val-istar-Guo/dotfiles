...
- When optimizing frontend interfaces, use `dev-browser` to connect to a browser and preview changes. Use `--connect` to attach to an existing browser instance unless the project requires a headless browser. If `--connect` fails, run the `chrome-debug` command first, then retry `dev-browser --connect` to connect to the browser for debugging. Iterate on code improvements based on visual feedback from the browser preview.
